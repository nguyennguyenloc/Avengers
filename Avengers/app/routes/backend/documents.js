...
const mainModel = require(__path_models + 'documents');
const categoryModel = require(__path_models + 'category');
const validatorDocuments = require(__path_validates + 'documents');
const utilsHelpers = require(__path_helpers + 'utils');
const fileHelpers = require(__path_helpers + 'file');
const paramsHelpers = require(__path_helpers + 'params');
const linkIndex = '/' + systemConfig.prefixAdmin + '/documents/';
const pageTitleIndex = 'Documents Mannagement';
const pageTitleAdd = pageTitleIndex + ' - Add';
const pageTitleEdit = pageTitleIndex + ' - Edit';
const folderView = __path_views_admin + 'pages/documents/';
const uploadThumb = fileHelpers.upload('thumb','documents');
...
router.get('(/status/:status)?', async(req, res, next) => {
  ...
  let statusFilter = await utilsHelpers.createFilterStatus(params.currentStatus, 'documents');
...
router.post('/save', (req, res, next) => {
  uploadThumb(req, res, async (errUpload) => {
    ...
    let errors = validatorDocuments.validator(req, errUpload, taskCurrent);
    
    if (errors.length > 0) {
      let pageTitle = (taskCurrent == "add") ? pageTitleAdd : pageTitleEdit;
      if(req.file != undefined) fileHelpers.remove('public/uploads/documents/', req.file.filename);
      let categoryItems = [];
      ...
      if(req.file == undefined){
        ...
        if(taskCurrent == "edit") fileHelpers.remove('public/uploads/documents/', item.image_old);
      }
